...
object OAuthUtils {
    ...
    //var authorizedEvent: PublishSubject<Boolean?> = PublishSubject.create()

    /**
     * Retrieves the o auth token, may be null beceause there is none
     *
    ...
    fun peekOAuthToken(context: Context?): OAuthToken? {
        ...
        return Authenticator.retrieveOAuth(context)
    }
     ...
     * @param jlsResponse the callback of the async call
     */
    fun fetchNewTokens(repository: Repository?,
                       context: Context,
                       username: String,
                       password: String,
                       jlsResponse: JlsCallback<OAuthToken>) {

        if (repository == null) {
            return
        }

        repository.getNewTokens(context, username, password, jlsResponse)
    }
    ...
    fun fetchNewOAuthAccessTokenBlocking(repository: Repository?, context: Context?): String? {
        ...
        try {
            ...
            val response = repository.getNewAccessTokenBlocking(context, oldToken)
